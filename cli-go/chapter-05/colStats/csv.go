...
func csv2float(r io.Reader, column int) ([]float64, error) {
	...
	// enable its ReuseRecord option to reuse the same slice for
	// each read operation, reducing the memory allocation even more.
	cr := csv.NewReader(r)
	cr.ReuseRecord = true
	...
	for i := 0; ; i++ {
		// Change method Read() for ReadAll() to reduce memory allocation
		row, err := cr.Read()
